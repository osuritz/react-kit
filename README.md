...
- **[use-debounce](src/hooks/use-debounce/README.md)** — drop-in generic
  debounce hook: returns a value only once it has stopped changing for
  `delay` ms. The primitive behind debounced search — React Query/SWR users
  should use it directly (debounce the query, put it in the `queryKey`, let
  the library own fetch state). No runtime dependencies.

- **[use-fetch](src/hooks/use-fetch/README.md)** — drop-in one-shot fetch
...
- **[use-autocomplete](src/hooks/use-autocomplete/README.md)** — drop-in
  ...
  loading/error state, stale-response discard, empty-query short-circuit,
  and an always-fresh `fetchFn` ref so inline arrow functions work without
  `useCallback`. Works with any Promise-returning backend; composes
  [use-debounce](src/hooks/use-debounce/README.md) (vendored in-folder).
