...
import { Cog, FileText, HelpCircle, LogOut, Moon, Plus, Search, Sun } from 'lucide-react';
import { Button } from '~/components/ui/button.tsx';
import { ActionsProvider, useAction, type Action } from '#hooks/action-registry/actions.tsx';
import {
  CommandPalette,
  type CommandSource,
      ...
      search: (query, signal) =>
        new Promise<Action[]>((resolve, reject) => {
          const timer = window.setTimeout(() => {
            ...
            const hits = FAKE_DOCS.filter((d) => d.toLowerCase().includes(lc)).slice(0, 5);
            resolve(
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <p className="text-muted-foreground text-xs">
          Or press <ModKbd>K</ModKbd> from anywhere on the page. Try typing “theme”, “sign”, or
          “onboarding” (the last hits the async docs source).
        </p>
      </div>
