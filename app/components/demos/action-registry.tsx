...
function RegistryList() {
  ...
  if (actions.length === 0) {
    return <p className="text-muted-foreground text-sm italic">No actions registered.</p>;
  }
  return (
        ...
        return (
          <li key={a.id} className="flex items-center gap-3 px-3 py-2 text-sm">
            ...
            <span className="flex-1">
              ...
              {a.group && <span className="text-muted-foreground ml-2 text-xs">· {a.group}</span>}
            </span>
            <code className="text-muted-foreground font-mono text-xs">{a.id}</code>
            {sc && (
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      ...
      <label className="text-muted-foreground inline-flex items-center gap-2 text-xs">
        ...
        />
        Mount <code className="font-mono">nav.search</code> — toggling proves subscribers re-render
        the list.
      </label>
    </div>
  );
