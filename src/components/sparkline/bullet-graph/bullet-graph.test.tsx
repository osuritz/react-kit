...
describe("BulletGraph", () => {
  it("renders one qualitative band per range segment (ranges.length + 1)", () => {
    const { container } = render(
      <BulletGraph value={7.2} target={8} ranges={[5, 7]} max={10} />,
    );
    expect(
      svgOf(container).querySelectorAll("rect[data-part='range']").length,
    ).toBe(3);
  });
  ...
  it("sizes the viewBox and exposes the aria-label", () => {
    const { container, getByRole } = render(
      <BulletGraph value={7.2} target={8} ranges={[5, 7]} max={10}
        width={180} height={24} label="Q2 bookings vs target" />,
    );
  ...
  it("produces no NaN geometry when everything is zero", () => {
    const { container } = render(
      <BulletGraph value={0} target={0} ranges={[]} max={0} />,
    );
